...
- add CLI argument parsing to `heldKarp_algorithm.py` so you can pass `--W`, `--T`, `--start`, `--k` from the command line;
- or add a unit test that verifies the DP on a small synthetic matrix.
